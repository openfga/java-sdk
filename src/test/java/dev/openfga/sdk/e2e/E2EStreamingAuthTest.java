...
public class E2EStreamingAuthTest {
    ...
    // The mock-oauth2-server token endpoint URL as seen from the *host*.
    // mock-oauth2-server exposes its token endpoint at /{issuerId}/token.
    // We set this as the apiTokenIssuer with the full path so the SDK
    // doesn't append its default "/oauth/token" suffix.
    private static final String OAUTH2_TOKEN_ENDPOINT = "http://host.docker.internal:9090/default/token";
    ...
    private static void testApiTokenAuth() throws Exception {
        ...
        // Create store
        var store = client.createStore(new CreateStoreRequest().name("e2e-apitoken-test"))
                .get();
    ...
    private static void testClientCredentialsAuth() throws Exception {
        ...
        // Create store
        var store = client.createStore(new CreateStoreRequest().name("e2e-oidc-test"))
                .get();
        String storeId = store.getId();
        check("createStore", storeId != null && !storeId.isEmpty());
        System.out.println("       Store ID: " + storeId);
